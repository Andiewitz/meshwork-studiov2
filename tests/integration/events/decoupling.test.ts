...
describe('EventBus & Decoupling Integration Tests', () => {
    ...
    describe('user.deleted event cascade', () => {
        beforeEach(() => {
            ...
            eventBus.on('user.deleted', async ({ id, tx }) => {
                await mockWorkspaceStorage.deleteAllUserData(id.toString(), tx);
            });
            eventBus.on('user.deleted', async ({ id, tx }) => {
                await mockCanvasStorage.deleteAllUserData(id.toString(), tx);
            });
            eventBus.on('user.deleted', async ({ id, tx }) => {
                await mockTeamStorage.deleteAllUserData(id.toString(), tx);
            });
        });

        it('should cascade user deletion to all dependent modules', async () => {
            const userId = 123;
            const mockTx = { transaction: 'mock' };
            ...
            expect(mockWorkspaceStorage.deleteAllUserData).toHaveBeenCalledWith('123', mockTx);

            expect(mockCanvasStorage.deleteAllUserData).toHaveBeenCalledTimes(1);
            expect(mockCanvasStorage.deleteAllUserData).toHaveBeenCalledWith('123', mockTx);

            expect(mockTeamStorage.deleteAllUserData).toHaveBeenCalledTimes(1);
            expect(mockTeamStorage.deleteAllUserData).toHaveBeenCalledWith('123', mockTx);
        });

        it('should pass tx=undefined when no transaction is provided', async () => {
            await eventBus.emitAsync('user.deleted', { id: 456 });

            expect(mockWorkspaceStorage.deleteAllUserData).toHaveBeenCalledWith('456', undefined);
            expect(mockCanvasStorage.deleteAllUserData).toHaveBeenCalledWith('456', undefined);
            expect(mockTeamStorage.deleteAllUserData).toHaveBeenCalledWith('456', undefined);
        });
    });
    ...
    describe('workspace.duplicated event cascade', () => {
        ...
        it('should duplicate canvas data when a workspace is duplicated', async () => {
            await eventBus.emitAsync('workspace.duplicated', { originalId: 10, newId: 20 });
    ...
    describe('Error isolation', () => {
        it('should not crash the emitter if one listener throws', async () => {
            ...
            eventBus.on('user.deleted', async ({ id }) => {
                await mockCanvasStorage.deleteAllUserData(id.toString());
            });
    ...
    describe('AppRegistry + EventBus integration', () => {
        ...
        it('should throw when retrieving an unregistered service', () => {
            const registry = new AppRegistry();
            expect(() => registry.get('nonexistent')).toThrow('Service nonexistent not found');
        });

        it('should throw when registering a duplicate service', () => {
            const registry = new AppRegistry();
            registry.register('auth', vi.fn());
            expect(() => registry.register('auth', vi.fn())).toThrow('already registered');
        });
    });
});
