...
import { loginSchema, registerSchema, changePasswordSchema } from '@server/modules/auth/schemas';
import { validatePasswordStrength, PASSWORD_POLICY } from '@shared/auth';

// Mock bcrypt for password hashing tests
vi.mock('bcrypt', () => ({
  default: {
    hash: vi.fn().mockImplementation(async (pwd: string, rounds: number) => `bcrypt:${rounds}:${pwd}`),
    compare: vi.fn().mockImplementation(async (pwd: string, hash: string) => hash === `bcrypt:12:${pwd}`),
  },
}));
...
vi.mock('jsonwebtoken', () => ({
  default: {
    sign: vi.fn().mockImplementation((payload: any, _secret: string, opts?: any) => {
...
describe('JWT Token Generation and Verification', () => {
  ...
  it('should_include_jti_in_refresh_token', () => {
    const { refreshToken, jti } = generateTokens({ id: 'user-123' });
    const payload = verifyToken(refreshToken, 'refresh');
